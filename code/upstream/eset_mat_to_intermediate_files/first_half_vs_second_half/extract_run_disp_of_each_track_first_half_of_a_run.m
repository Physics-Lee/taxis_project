...
function run_disp_of_all_tracks = extract_run_disp_of_each_track_first_half_of_a_run(eset)
            ...
            for run = track.run
                if screen_a_run(run)
                    if run.runTime > t_threshold
                        ...
                        run_disp_of_a_track{end+1,1} = run.track.dq.displacement(:,start_ind:half_ind) / 22; % mm
                    end
                end
            end
