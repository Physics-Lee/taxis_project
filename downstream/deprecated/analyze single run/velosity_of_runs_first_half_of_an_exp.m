function velosity_first_half_of_an_exp = velosity_of_runs_first_half_of_an_exp(eset)

    velosity_first_half_of_an_exp = []; 
    for expt = eset.expt
                ...
                for run = track.run                    
                    if screen_a_run(run)                        
                        ...
                        if start_ind + track.startFrame <= half_ind
                            velosity_first_half_of_an_exp(end+1) = (run.pathLength/22)/run.runTime; % mm/s
                        end
                    end
                end
            end            
