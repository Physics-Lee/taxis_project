...
function [run_disp_second_half_of_an_exp,mask] = extract_run_disp_second_half_of_an_exp(eset)

run_disp_second_half_of_an_exp = cell(1,1);
count = 0;
mask = [];
            ...
            for k = 1:length(track.run)
                ...
                if screen_a_run(run)
                    ...
                    if start_ind + track.startFrame > half_ind
                        mask(end) = 1;
                        count = count + 1;
                        run_disp_second_half_of_an_exp{count,1} = run.track.dq.displacement(:,start_ind:end_ind);
                    end
                end
            end
        end
