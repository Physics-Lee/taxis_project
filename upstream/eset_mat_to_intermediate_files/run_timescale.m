% extract run disps (mm) of the first half and the second half of a run
%
...
if path ~= 0
    ...
    if tf==1
        ...
        for i = indx
            ...
            save_folder_path = fullfile(folder_path_to_eset,'run_disp_of_each_track_first_half_of_a_run');
            create_folder(save_folder_path);

            % extract
            run_disp_of_all_tracks = extract_run_disp_of_each_track_first_half_of_a_run(eset);
            ...
            % create save folder
            folder_path_to_eset = fileparts(full_path);
            save_folder_path = fullfile(folder_path_to_eset,'run_disp_of_each_track_second_half_of_a_run');
            create_folder(save_folder_path);

            % extract
            run_disp_of_all_tracks = extract_run_disp_of_each_track_second_half_of_a_run(eset);
